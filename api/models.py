from django.db import models
import string
import random

def generate_unique_code():
    length=6

    while True:
        code =''.join(random.choices(string.ascii_uppercase, k=length))
        if Room.objects.filter(code=code).count() == 0:
            break
    return code

# Create your models here.
class Room(models.Model):
    '''Room Model to hold the details of the room
    and inside this class we will define the fields of the room
    and code, host, guest_can_pause, votes_to_skip
    and created_at are the objects of the room
    and this will be used to create the room
    '''
    code = models.CharField(max_length=8,default=generate_unique_code ,unique=True)
    host = models.CharField(max_length=50, unique=True)
    guest_can_pause = models.BooleanField(null=False, default=False)
    votes_to_skip = models.IntegerField(null= False, default=1)
    created_at = models.DateTimeField(auto_now_add=True) # this will automatically set the date when the object is created